...
This failure occurred during the 1.1 and 1.2 release investigations: Apple's `codesign` signed and notarized the app successfully, then returned either `No such process` or `host has no guest with the requested attributes` while signing an otherwise valid DMG on hosted macOS runners. The failure reproduced on clean macOS 15 and macOS 26 arm64 runners. The normal workflow therefore uses the checksum-pinned open-source `rcodesign` implementation only for the outer DMG, then validates the resulting signature with Apple's `codesign`, checks the leaf certificate team against the app, notarizes and staples with Apple's tools, and requires Gatekeeper acceptance. The earlier 1.0 DMGs had actually been packaged locally; the `1.0` tag did not match the old workflow's `v*.*` trigger.
...
- `rcodesign` cannot produce an Apple-verifiable signature with the expected certificate team: recover by signing the DMG on a trusted maintainer Mac.
