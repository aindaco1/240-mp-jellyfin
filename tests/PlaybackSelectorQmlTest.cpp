...
class PlaybackSelectorQmlTest final : public QObject {
    Q_OBJECT

private slots:
    void longValueStaysBetweenControls();
};

void PlaybackSelectorQmlTest::longValueStaysBetweenControls()
{
    QQmlEngine engine;
    const QUrl componentUrl = QUrl::fromLocalFile(QStringLiteral(
        TEST_SOURCE_ROOT "/views/Components/PlaybackSelectorRow.qml"));
    ...
    selector->setProperty("label", QStringLiteral("Audio"));
    selector->setProperty("value", QStringLiteral(
        "A VERY LONG SOUNDTRACK NAME THAT MUST NEVER COVER THE LABEL OR SELECTOR ARROWS.FLAC"));
    ...
    const qreal previousRight = previous->mapToItem(
        selectorItem, QPointF(previous->width(), 0)).x();
    ...
    const qreal nextRight = next->mapToItem(
        selectorItem, QPointF(next->width(), 0)).x();

    QVERIFY(previous->mapToItem(selectorItem, QPointF(0, 0)).x() > labelRight);
...
QTEST_MAIN(PlaybackSelectorQmlTest)
#include "PlaybackSelectorQmlTest.moc"
