...
- Local queue state contains only root-contained local paths, entry UUIDs, bounded display/error text, and track choices. It is schema/size bounded and owner-only. Playlist import must reject remote schemes, root escapes, cycles, excessive nesting, newline paths, and unsupported files before writing `local_queue.json` or `local_queue.m3u8`.
